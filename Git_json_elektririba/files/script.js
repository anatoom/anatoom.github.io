...
svg.selectAll(".tooltip-date,.tooltip-hind,.tooltip-value")
...
//Read the data
d3.csv("https://www.delfi.ee/misc/common/embed/electricity/prices.php?csv=1",
  // When reading the csv, I must format variables:
  function(d){
    return { time : d3.timeParse("%H:%M:%S")(d.time), value1 : d.value1.replace(/,/g, '.'), value2 : d.value2.replace(/,/g, '.')  }
  },
...
var maxHind = d3.max(data, function(d) { return Math.max(d.value1); });
var minHind = d3.min(data, function(d) { return Math.min(d.value1); });
var keskHind = d3.median(data, function(d) { return (d.value1); });
   ...
   // Add Y axis
    var y = d3.scaleLinear()
      .domain([d3.min(data, function(d) {
        return Math.min(d.value1, d.value2) - 50; }), d3.max(data, function(d) {
...
innerSVG
      ...
      .attr("class", function(d) {
        ...
        .attr("fill", function(d) {
           ...
           // if (d.value1 == d3.max(data, function(d) { return d.value1; })) {return "red"}
            else 	{ return "#55ABC0"}
        ;})
...
focus.attr("transform", "translate(" + x(d.time) + "," + y(d.value1) + ")");
focus.select(".tooltip-date").text(displayTime(d.time));
focus.select(".tooltip-value").text((d.value1));
